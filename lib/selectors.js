...
export const selectConnectionMode = state => get(state, "user.mode", "none")
export const selectDemoTutorialStatus = state => get(state, "demo.status", "idle")
export const selectDemoTutorialStep = state => get(state, "demo.currentStep", 0)
...
const selectAllOpenOrders = createSelector(
  ...
  (allOrders, cancelledOrders, filledOrders) => {
    allOrders = (allOrders || []).filter(Boolean)
    cancelledOrders = (cancelledOrders || []).filter(Boolean)
    filledOrders = (filledOrders || []).filter(Boolean)
...
export const selectMyOpenOrders = createSelector(
  ...
  (orders, account, market) => {
    if (!Array.isArray(market) || market.length < 2) return []
